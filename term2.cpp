...
void precomputeCurvedMetrics(const GriMesh& mesh, int order,
	...
	for (int ib = 0; ib < mesh.num_boundary_faces; ++ib) {
		...
		int nnode = mesh.BedgeNodeOffset[(size_t)ib + 1] - mesh.BedgeNodeOffset[(size_t)ib];
		bool is_quadratic_curve = (nnode == 3);
		...
		if (is_quadratic_curve) {
			...
			Eigen::Matrix2d J = Jacobian(mesh, elem);
			double detJ = abs(J.determinant());
			Eigen::Matrix2d adj = J.inverse() * detJ;
			em.detJ[q] = detJ;
			em.G[q] = adj;
		}
...
void addTerm2(const GriMesh& mesh, double* R, int order,
	...
	for (int k = 0; k < mesh.Ne; ++k) {
		...
		if (!is_curved) {
			...
			G_const = J_const.inverse() * abs(J_const.determinant());
		}
